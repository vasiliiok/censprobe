...
**Caveat для `mtproto_orig` + per-vantage diagnostic.** Оригинальный mtproto-proxy (TelegramMessenger/MTProxy, C) не отвечает на handshake локально — он прокидывает MTProto-frame в реальный Telegram DC (адреса в baked-in `proxy-multi.conf`). Probe ждёт `resPQ` именно от DC, поэтому если у listener'а заблокирован outbound к Telegram DC IPs, сессия проявится как BLOCKED.

Две distinct failure shapes на RU vantage:

1. **Preflight prune (2026-05-13).** Перед запуском `mtproto-proxy` listener TCP-пробует каждый IP из `proxy-multi.conf` на 8888 параллельно. Недостижимые выкидываются — на старт даётся pruned-копия. Это убирает auth_cluster reconnect storm на хостах где TSPU режет TCP/8888 (наблюдался hot loop 145 connect/s, ~35% CPU простоя, 0 `accept4()`). Если **0** upstream'ов alive — daemon вообще не запускается, responder помечается `unavailable=True`. Prune itself is positive network-block evidence — Telegram DC fleet unreachable от vantage.
2. **Post-start self-test (2026-05-12).** После запуска daemon listener делает loopback `probe_mtproto_orig` против самого себя (12 s budget). Если падает (K/N alive at prune, но daemon wedged) — `responder_self_test_ok=False` прокидывается в `ProtocolResult`. На RU это L7 ТСПУ filtering auth_cluster RPC heartbeat (TCP/8888 SYN проходит prune, payload фильтруется); на non-RU vantage может быть local daemon issue.
3. **Финализация (2026-05-14 design — unified BLOCKED).** Оба shape производят `Verdict.BLOCKED` (раньше — `ERROR` через self-test downgrade). Reasoning: client-side experience тоже BLOCKED (он реально не может подключиться), final cross-verify тоже BLOCKED, оператор видит coherent вердикт. Per-protocol `_mtproto_orig_failure_note` пишет one of two diagnostic strings в `ProtocolResult.note`: `"mtproto-proxy not launched: 0/N upstreams reachable… confirmed unreachable from this vantage"` (Shape 1) либо `"mtproto-proxy launched with K/N alive but loopback self-test still timed out … on RU vantages this is L7 ТСПУ filtering auth_cluster RPC; on non-RU it can be local daemon issue"` (Shape 2). Note surfaced в дашборде `04 — Protocol Reachability` колонке `Diagnostic`.

Earlier (до 2026-05-14) finalize() даунгрейдил BLOCKED→ERROR на failed self-test, чтобы strictly preserve "BLOCKED ≡ confirmed block" invariant. Practice showed что на RU vantage wedge всегда был downstream effect TSPU — same end-to-end blocking как у клиента — так что ERROR создавал inconsistency (client писал BLOCKED, listener ERROR). Hybrid design выкидывает downgrade, оставляет `responder_self_test_ok` как diagnostic-only signal, attribution уезжает в note. Invariant остаётся satisfied: prune evidence для Shape 1 IS confirmed block, для Shape 2 — operator interprets note vs vantage location.

Verified live на трёх вантажах: vultr Frankfurt (вне TSPU) — `mtproto_orig OK`; GCP Frankfurt + Yandex Cloud RU + MTS RU (внутри TSPU) — `mtproto_orig BLOCKED` с конкретной диагностикой L7 vs prune-zero-alive.
