...
**Cross-verification.** После всех probes клиент дополнительно фетчит `/snapshot` с listener'а (тот же bearer-token, multi-serve) и печатает таблицу `Client | Listener | Final` per-protocol. Listener — authoritative источник (kernel-level counters, HMAC-валидация, iptables). Если client-вердикт расходится с listener — final = listener, в колонке `Note` появится `client overread (listener saw less)` либо `listener saw data the client missed`. По умолчанию final-вердикт `CONNECTED` означает "протокол реально работает"; `HANDSHAKE_ONLY` и `BLOCKED` оба означают "не работает" (различие диагностическое: HANDSHAKE_ONLY ⇒ DPI режет data plane после рукопожатия; BLOCKED ⇒ блок на L3/L4 либо silent drop сразу после init). Для mtproto-семейства `length_timeout_post_init` и wedged-responder shape: с 2026-05-14 verdict стал unified BLOCKED (был ERROR через self-test downgrade) — оператор-experience и client-verdict совпадают; L7-vs-local атрибуция уехала в поле `note`. Подробнее про инвариант `BLOCKED ≡ confirmed network block` и про две failure-shapes mtproto_orig — в разделе ниже.

**Asymmetric DPI detection (2026-05-13, broadened 2026-05-14).** Если `client=BLOCKED` с одним из read-timeout-after-handshake маркеров (`welcome_read_timeout`, `orig_resPQ_len_timeout`, `orig_resPQ_body_timeout`, `orig_resPQ_truncated`) **AND** listener-сторона видит handshake-уехавший (`listener=OK` либо `listener=HANDSHAKE_ONLY`), final понижается до `HANDSHAKE_ONLY` с note `asymmetric DPI: server replied but client did not receive`. Сценарий: сервер отправил ответ (iptables PSH-ACK счётчик тикнул), но return-leg к клиенту был дропнут — DPI режет server→client направлено. Подтверждено на MTS RU vantage 2026-05-13 для mtg-faketls. HANDSHAKE_ONLY-ветка покрывает каноничный MTS-ingress shape: daemon отдал SERVER_HELLO (PSH-ACK=1), клиент ответ не получил, return traffic от клиента не пришёл — `listener.finalize()` приземлил HANDSHAKE_ONLY (handshake_count=1, data_transfer_ok=False), и теперь cross-verify атрибутирует это к asymmetric DPI вместо generic `client=BLOCKED` fall-through note. Для других client-ошибок (`connection_refused`, `tcp_timeout` и т.п.) listener-OK по-прежнему — authoritative, так как они означают «клиент вообще не доехал до сервера» (Docker-loopback / host-firewall артефакт, не цензура).

**Display колонки в client-таблице.** Per-protocol строка показывает `Elapsed` (общее wall-clock время пробы, всегда корректное даже для timeout-вердиктов) и `RTT` (протоколо-специфичный сигнал: TCP-connect для TCP-проб, tunnel ICMP-ping для VPN). До 2026-05 рендерилось только `rtt_ms` как `(Xms)` рядом с вердиктом — что вводило в заблуждение на mtproto timeout-вердиктах, где `rtt_ms = TCP connect time` (~10 ms), а реальная проба висела 15 секунд. Теперь обе колонки видны раздельно.
...
`ProtocolSpec` несёт ещё три булевых атрибута, влияющих на лайфсайкл:
- `is_mtg_protocol` — true для `mtproto_proxy` / `mtproto_proxy_alt` (mtg fakeTLS responder), драйвер cap_at HANDSHAKE_ONLY для mtg-DC-unreachable;
- `requires_telegram_dc` — true для всех трёх MTProto-вариантов; гейтит `LiveSnapshot.dc_reach_ok` инъекцию;
- `uses_iptables_counter` — true для `openvpn` + всех MTProto-вариантов; гейтит `LiveSnapshot.data_counters_available` так, что rootless-listener без CAP_NET_ADMIN не даунгрейдит client=OK до HANDSHAKE_ONLY на структурно-неработающем счётчике.
...
**Caveat для `mtproto_orig` + per-vantage diagnostic.** Оригинальный mtproto-proxy (TelegramMessenger/MTProxy, C) не отвечает на handshake локально — он прокидывает MTProto-frame в реальный Telegram DC (адреса в `proxy-multi.conf`, который Telegram рекомендует обновлять не реже раза в сутки). Listener при старте делает best-effort fetch свежей пары `proxy-multi.conf` + `proxy-secret` с `core.telegram.org`, fallback на baked-in копию при сетевой ошибке. Probe ждёт `resPQ` именно от DC, поэтому если у listener'а заблокирован outbound к Telegram DC IPs, сессия проявится как BLOCKED.

Три distinct failure shapes (определяются в preflight + start()):

1. **Preflight prune.** Перед запуском `mtproto-proxy` listener TCP-пробует каждый IP из `proxy-multi.conf` на 8888 параллельно. Недостижимые выкидываются — на старт даётся pruned-копия. Это убирает auth_cluster reconnect storm на хостах где TSPU режет TCP/8888 (наблюдался hot loop 145 connect/s, ~35% CPU простоя, 0 `accept4()`). Если **0** upstream'ов alive — daemon вообще не запускается, responder помечается `unavailable=True`. Prune itself is positive network-block evidence — Telegram DC fleet unreachable от vantage.
2. **NAT-detection (2026-05-16).** Cloud VMs (GCP/AWS/DO private VPC) сидят за 1:1 NAT — daemon видит private source IP (10.x.y.z), но upstream Telegram DC видит NAT-translated public IP. Без `--nat-info <private>:<public>` оригинальный C MTProxy зашивает local IP в auth_cluster RPC handshake, upstream обнаруживает mismatch и silently дропает соединение после nonce exchange (наблюдался 92% disconnect rate). Responder при старте вызывает `detect_nat_pair()` и добавляет `--nat-info` автоматически когда egress private != public; на bare-metal hosts с directly-attached public IP флаг не передаётся.
3. **Post-start self-test.** После запуска daemon listener делает loopback `probe_mtproto_orig` против самого себя (12 s budget). Если падает (K/N alive at prune, NAT-info applied, но daemon wedged) — `responder_self_test_ok=False` прокидывается в `ProtocolResult`. На RU vantage это L7 ТСПУ filtering auth_cluster RPC heartbeat (TCP/8888 SYN проходит prune, payload фильтруется); на non-RU vantage если NAT-detection не сработал (например, обе SYN-echo сервисы недоступны) — это пропавший `--nat-info`.

**Финализация (2026-05-14 design — unified BLOCKED).** Любой из shape'ов производит `Verdict.BLOCKED` (раньше — `ERROR` через self-test downgrade). Reasoning: client-side experience тоже BLOCKED, final cross-verify тоже BLOCKED, оператор видит coherent вердикт. Per-protocol `_mtproto_orig_failure_note` пишет диагностический string в `ProtocolResult.note`: `"mtproto-proxy not launched: 0/N upstreams reachable… confirmed unreachable from this vantage"` (Shape 1) либо `"mtproto-proxy launched with K/N alive but loopback self-test still timed out … on RU vantages this is L7 ТСПУ filtering auth_cluster RPC; on non-RU it means NAT detection failed at startup"` (Shape 2 / 3). Note surfaced в дашборде `04 — Protocol Reachability` колонке `Diagnostic`.

Earlier (до 2026-05-14) finalize() даунгрейдил BLOCKED→ERROR на failed self-test, чтобы strictly preserve "BLOCKED ≡ confirmed block" invariant. Practice showed что на RU vantage wedge всегда был downstream effect TSPU — same end-to-end blocking как у клиента — так что ERROR создавал inconsistency (client писал BLOCKED, listener ERROR). Hybrid design выкидывает downgrade, оставляет `responder_self_test_ok` как diagnostic-only signal, attribution уезжает в note. Invariant остаётся satisfied: prune evidence для Shape 1 IS confirmed block, для Shape 2/3 — operator interprets note vs vantage location.

Verified live на нескольких вантажах: vultr Frankfurt (вне TSPU) — `mtproto_orig OK`; GCP southamerica1 (CL) и us-central1 (US) после NAT fix 2026-05-16 — `mtproto_orig OK`; Yandex Cloud RU + MTS RU (внутри TSPU) — `mtproto_orig BLOCKED` с конкретной диагностикой L7 vs prune-zero-alive.
...
Единственное намеренное исключение — `.env`. Закоммичен в репо с safe defaults для loopback-only сервисов (`DOCKERHUB_USERNAME`/`DOCKERHUB_TAG`, `CREDS_PORT`, `SYNC_PORT`, `DB_PASSWORD` (случайный 32-hex), `GRAFANA_PASSWORD=admin`, `RUNS_COUNT`, `CENSPROBE_IMPORT_INTERVAL_SEC`), чтобы `docker compose up` работал out of the box. Полная таблица значений — в [Переменные окружения](#переменные-окружения-env); перегенерируйте `DB_PASSWORD`/`GRAFANA_PASSWORD` в своём working tree перед удалённым деплоем.
...
sync-api сам по себе HTTP-аутентификации не имеет — он биндится на `127.0.0.1:8080` (loopback), Grafana ходит за данными в Postgres напрямую (не через эти endpoint'ы), и модель развёртывания предполагает, что хост принадлежит оператору. Если когда-нибудь понадобится выставить порт 8080 наружу — ставьте reverse proxy с auth перед ним, сам FastAPI на internet-facing работу не рассчитан.
