...
**Asymmetric DPI detection (2026-05-13).** Если `listener=OK` + `client=BLOCKED` с одним из read-timeout-after-handshake маркеров (`welcome_read_timeout`, `orig_resPQ_len_timeout`, `orig_resPQ_body_timeout`, `orig_resPQ_truncated`), final понижается до `HANDSHAKE_ONLY` с note `asymmetric DPI: server replied but client did not receive`. Этот сценарий означает что сервер действительно отправил ответ (listener-side iptables PSH-ACK счётчик тикнул), но return-leg к клиенту был дропнут — DPI режет server→client направлено. Подтверждено на MTS RU vantage 2026-05-13 для mtg-faketls (`mtproto_proxy` + `mtproto_proxy_alt`). Без этого даунгрейда listener-OK перебивал бы client-BLOCKED, маскируя реально неработающий протокол как «всё хорошо». Для других client-ошибок (`connection_refused`, `tcp_timeout` и т.п.) listener-OK по-прежнему — authoritative, так как они означают «клиент вообще не доехал до сервера» (Docker-loopback / host-firewall артефакт, не цензура).
