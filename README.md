...
**Cross-verification.** После всех probes клиент дополнительно фетчит `/snapshot` с listener'а (тот же bearer-token, multi-serve) и печатает таблицу `Client | Listener | Final` per-protocol. Listener — authoritative источник (kernel-level counters, HMAC-валидация, iptables). Если client-вердикт расходится с listener — final = listener, в колонке `Note` появится `client overread (listener saw less)` либо `listener saw data the client missed`. По умолчанию final-вердикт `CONNECTED` означает "протокол реально работает"; `HANDSHAKE_ONLY` и `BLOCKED` оба означают "не работает" (различие диагностическое: HANDSHAKE_ONLY ⇒ DPI режет data plane после рукопожатия; BLOCKED ⇒ блок на L3/L4 либо silent drop сразу после init). Для mtproto-семейства `length_timeout_post_init` (TCP жив, init принят, но resPQ через DC не вернулся) классифицируется как BLOCKED **только если listener-side self-test того же протокола успешно прошёл** — иначе вердикт даунгрейдится до ERROR с диагностическим note. Подробнее про инвариант `BLOCKED ≡ confirmed network block` — в разделе про mtproto_orig ниже.
...
**Caveat для `mtproto_orig` + автозащита от false-BLOCKED.** Оригинальный mtproto-proxy (TelegramMessenger/MTProxy, C) не отвечает на handshake локально — он прокидывает MTProto-frame в реальный Telegram DC (адреса в baked-in `proxy-multi.conf`). Probe ждёт `resPQ` именно от DC, поэтому если у listener'а заблокирован outbound к Telegram DC IPs (а у самого клиента — нет), сессия может выглядеть как BLOCKED не из-за client-side DPI.

Чтобы соблюсти строгий инвариант «вердикт BLOCKED ≡ подтверждённая сетевая блокировка», listener делает три уровня защиты:

1. **Preflight prune (2026-05-13).** Перед запуском `mtproto-proxy` listener TCP-пробует каждый IP из `proxy-multi.conf` на 8888 параллельно. Недостижимые выкидываются — на старт даётся pruned-копия. Это убирает auth_cluster reconnect storm на хостах где TSPU режет TCP/8888 (наблюдался hot loop 145 connect/s, ~35% CPU простоя, 0 `accept4()`). Если **0** upstream'ов alive — daemon вообще не запускается, responder помечается `unavailable=True`.
2. **Post-start self-test (2026-05-12).** После запуска daemon listener делает loopback `probe_mtproto_orig` против самого себя (12 s budget). Если падает — `responder_self_test_ok=False` прокидывается в `ProtocolResult`.
3. **Финализация с downgrade.** `ProtocolResult.finalize()` автоматически даунгрейдит BLOCKED → ERROR если self-test провалился или responder unavailable, с конкретной диагностикой в поле `note`: `"mtproto-proxy not launched: 0/N upstreams reachable…"` (vantage не достучался до Telegram), либо `"mtproto-proxy launched with K/N upstreams alive but the listener-side loopback self-test still timed out after 12 s — likely L7 censor blocking auth_cluster RPC heartbeat"` (TCP проходит, но L7-payload фильтруется). Тот же downgrade применяется и на client-стороне в cross-verification таблице через делегирование `finalize()`.

Verified live на трёх вантажах: vultr Frankfurt (вне TSPU) — `mtproto_orig OK`; GCP Frankfurt + Yandex Cloud RU (внутри TSPU когда сосёт outbound к Telegram DC) — `mtproto_orig ERROR` с конкретной диагностикой, не false-BLOCKED.
