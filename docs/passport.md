...
Censorship Probe — распределённая система из шести независимых Docker-контейнеров, координирующихся через GitHub-репозиторий. Система измеряет:
...
- **Часть 3** — Системная архитектура: шесть контейнеров.
- **Часть 4** — Test ID: идентификация испытаний.
- **Части 5–9** — Спецификации пяти основных контейнеров (reporter описан в Части 11).
- **Часть 10** — Содержимое репозитория: baseline, targets, signatures.
- **Часть 11** — Что видно в Grafana. Reporter.
- **Часть 12** — Privacy и opsec.
...
> ВАЖНО: IP могут меняться. Рекомендуется периодически сверять актуальные адреса дата-центров с официальной документацией Telegram.
...
3. **Шесть Docker-контейнеров, один `docker-compose.yml`, профили.** На любой машине:
   ```
...
| 6 | `reporter` | Любая машина | Генерация HTML/PDF-отчётов из собранных данных. | После генерации |
...
### 5.6. Grafana-дашборды (набор)

Поставляются как JSON-файлы в `packages/dashboard/grafana/dashboards/`, provisioning через Grafana API при старте.
...
git pull
TEST_ID=selectel-spb-001 \
  docker compose --profile reporter run --rm reporter --pdf
# → HTML/PDF появится в ./reports/<test_id>/
```
...
## Приложение B. Полезные источники
