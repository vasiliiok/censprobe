...
- **Static analysis**: `ruff check + ruff format --check` (E/F/W/B/I/UP/S), `mypy strict` (полный `packages/*/src` tree, 53 файл), `bandit -ll` (Python security), `hadolint` (Dockerfile), `actionlint` (workflow YAML), `yamllint` (configuration YAML).
