...
**Probe-core total: 29 файлов, 385 тестов.**
...
| `censprobe_listener.preflight` | `unit/test_preflight.py` | 29 | conntrack health (`_check_conntrack` low-max + NOTRACK downgrade, high-water warn, missing /proc skip), `_check_iptables_capability` (no PATH = warn, rule-absent = ok proves CAP_NET_ADMIN, EPERM = warn with hint), `_cleanup_orphan_rules` (no rules → ok, orphans deleted via `-A→-D` conversion), `_check_telegram_dc_reach` (3 DC parallel TCP, all-unreachable warn), `_mtproxy_upstream_checkresult` (renders the single-source UpstreamProbe into a preflight CheckResult), `_parse_proxy_multi_upstreams` (canonical + malformed + dedup), `probe_all_proxy_multi_upstreams` (parallel TCP-probe of all upstreams, partitions alive/unreachable), `write_pruned_proxy_multi_conf` (mirrors default + emits proxy_for lines + asserts non-empty), `run_mtproxy_orig_self_test` (OK / BLOCKED→warn / timeout→warn), `run_preflight` orchestrator order |
...
| `censprobe_listener.mtproto_orig_responder` | `unit/test_mtproto_orig_responder.py` | 9 | secret parsing (dd-prefix stripped for `-S` argv), iptables OUTPUT PSH+ACK rule shape, idempotent re-install path, **prune-storm guard**: `start()` does NOT spawn subprocess when 0/N upstreams alive (`TestStartSkipsWhenAllUpstreamsUnreachable`), `unavailable=True` flag surfaces upstream counts, `stop()` is no-op when responder skipped, reuses pre-completed preflight `upstream_probe` instead of re-probing |
| `censprobe_listener.main._mtproto_orig_failure_note` | `unit/test_mtproto_orig_failure_note.py` | 5 | per-vantage diagnostic note: passes through `None` for OK self-test / unconfigured; "not launched: 0/N upstreams" when `unavailable=True`; "launched with K/N alive but loopback self-test still timed out — likely L7 censor on auth_cluster RPC or NAT-detection failed" when daemon wedged despite reachable upstreams |
| `censprobe_listener.main` dc-reach / iptables-cap gating | `unit/test_dc_reach_gate.py` | 20 | `_extract_dc_reach_ok` + `_extract_iptables_cap_ok` (ok→True / warn→False / skip→None / missing→None), `_data_counters_available` per-protocol routing through `ProtocolSpec.uses_iptables_counter`, `_finalize_protocol_result` mtg DC-unreachable cap_at HANDSHAKE_ONLY |
| `censprobe_core.ephemeral_cert.detect_nat_pair` | `unit/test_detect_nat_pair.py` (probe-core) | 7 | direct public IP → None, GCP/AWS 1:1 NAT shape → (private, public), RFC1918 172/12 + CGNAT 100.64/10 also classed unroutable, no-outbound → None, echo unreachable → None, echo == local → None |
...
| `censprobe_client.main` (cross-verification helpers) | `unit/test_cross_verification.py` | 16 | `_listener_verdict` mirrors `ProtocolResult.finalize` — `data_transfer_ok=True ⇒ OK` (cryptographic ground truth wins over log-parsed handshake counter), handshake-only-no-data ⇒ HANDSHAKE_ONLY, neither signal ⇒ BLOCKED; `_agreed_verdict` listener-wins matrix — both-OK no note, client-overconfident → "client overread", listener-OK + client-not → "listener saw data", other disagreements → `client=X` note; asymmetric-DPI detection: client=BLOCKED with read-timeout-after-handshake marker (`welcome_read_timeout`, `orig_resPQ_len_timeout`, `orig_resPQ_body_timeout`, `orig_resPQ_truncated`) + listener=OK *or* listener=HANDSHAKE_ONLY → final=HANDSHAKE_ONLY with note "asymmetric DPI: server replied but client did not receive" (the HANDSHAKE_ONLY branch covers the MTS-ingress mtg case where the daemon's SERVER_HELLO ticked the iptables PSH-ACK counter exactly once and no return traffic ever came back); other client errors (`connection_refused`, `tcp_timeout`) bypass downgrade |
...
|------|-------:|-------:|----------|
| `packages/probe-core/tests` | 34 | 453 | плотное (config, scoring, subcategories, runner, все 8 модулей измерений, credentials_reader, protocol_probes helpers + ping_echo + mtproto BLOCKED-on-timeout, `_stamp_elapsed`/`stamp_test_elapsed` decorators, ProtocolResult.finalize truth table + data_counters_available branch, `detect_nat_pair`) |
| `packages/solo/tests` | 1 | 1 | smoke-only |
| `packages/listener/tests` | 16 | 315 | credentials + AWG invariants (parametrize-heavy), preflight checks (incl. `probe_all_proxy_multi_upstreams`, `write_pruned_proxy_multi_conf`, dc-reach + iptables-cap gating), openvpn AND-gate + auth-bytes latch, mtproto-orig responder (incl. prune-storm guard, `--nat-info` flag wiring, preflight upstream-probe reuse), `_mtproto_orig_failure_note` (per-vantage diagnostics), cred_server `/snapshot` endpoint, `_generate_session_id` prefix encoding + SAFE_ID contract |
| `packages/client/tests` | 5 | 59 | smoke + cross-verification helpers (incl. asymmetric-DPI downgrade — client=BLOCKED with read-timeout-after-handshake marker + listener=OK *or* HANDSHAKE_ONLY → final=HANDSHAKE_ONLY, verified live MTS RU 2026-05-13; data-counters-unavailable promotion) + retry policy + cert pinning + snapshot schema drift |
...
| `tests/` (workspace) | 9 | 15 | contracts + snapshots + e2e (`e2e-dashboard` job, 2 deselected по дефолту) |
| **Total** | **76** | **1009** (после parametrize, 2 deselected по дефолту) | — |
