...
| `packages/client/tests` | 3 | 37 | smoke + cross-verification helpers (incl. asymmetric-DPI downgrade — client=BLOCKED with read-timeout-after-handshake marker + listener=OK *or* HANDSHAKE_ONLY → final=HANDSHAKE_ONLY, verified live MTS RU 2026-05-13) + retry policy (transient/permanent classification) |
...
| **Total** | **61** | **844** (после parametrize) | — |
