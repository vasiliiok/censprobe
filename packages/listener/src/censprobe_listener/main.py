...
async def _async_main(test_id: str, session_id: str, skip_push: bool) -> None:
    ...
    if not protocols_path.exists():
        ...
        if not skip_push:
            try:
                ...
                console.print("[green]✓ Credentials pushed — client can now git pull[/green]")
            except Exception as e:
                console.print(f"[yellow]Push failed: {e}. Client must use local protocols.yaml.[/yellow]")
    else:
    ...
    if not skip_push:
        try:
            ...
            console.print("[green bold]✓ Results pushed to GitHub[/green bold]")
        except Exception as e:
            console.print(f"[red]Push failed: {e}. Report saved locally.[/red]")
    else:
...
def _print_responder_status(responders: dict, errors: dict, creds: ProtocolCredentials) -> None:
    ...
    for name in all_protocols:
        ...
        if name in responders:
            ...
            table.add_row(name, port, f"[red]✗ {err[:40]}[/red]")
