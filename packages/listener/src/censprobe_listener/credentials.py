...
def creds_to_yaml(
    ...
    actually started — required, never inferred. Only those sections are
    emitted, so a protocol the operator removed from
    ``protocols.enabled`` (and therefore from ``protocols.ports``) does
    not show up in the YAML at all — the client sees the absence via
    ``_protocols_enabled`` and skips that probe. Listener and client
...
def _wg_keypair() -> tuple[str, str]:
    ...
    try:
        ...
        raise RuntimeError("wg binary not found — listener image is missing wireguard-tools") from e
...
def _reality_keypair() -> tuple[str, str]:
    ...
    """
    try:
        out = subprocess.check_output(["xray", "x25519"], stderr=subprocess.DEVNULL).decode()
    except FileNotFoundError as e:
        raise RuntimeError("xray binary not found — listener image is missing xray-core") from e
    except subprocess.CalledProcessError as e:
...
def _openvpn_static_key() -> str:
    ...
    try:
        ...
        raise RuntimeError("openvpn binary not found — listener image is missing openvpn") from e
    except subprocess.CalledProcessError as e:
