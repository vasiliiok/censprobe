...
class CredServer:
    ...
    def stop(self) -> None:
        ...
        if self._thread is not None:
            ...
            self._thread = None
        # Best-effort wipe of cert/key material from disk.
        ...
        class _Handler(http.server.BaseHTTPRequestHandler):
            ...
            def do_GET(self):  # noqa: N802 — http.server method name
                ...
                if not auth.startswith("Bearer "):
                    self._reject(401, "missing bearer token"); return

                presented = auth[len("Bearer "):]
                if not hmac.compare_digest(presented, srv.token):
                    self._reject(403, "invalid token"); return
...
def _query_public_ip_echo() -> str | None:
    ...
    for url in _PUBLIC_IP_ECHOES:
        try:
            with urllib.request.urlopen(url, timeout=_PUBLIC_IP_TIMEOUT_SEC) as r:
