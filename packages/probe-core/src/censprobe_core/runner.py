...
from censprobe_core.config import CensprobeConfig, get_config, load_config
...
class ProbeRunner:
    ...
    async def run_all(self, repeats: int | None = None) -> list[TestResult]:
        ...
        # Optional CLI override of every module's repeat count.
        if repeats is not None:
            for name in ("dns", "tcp", "tls", "http"):
                ...
                if section is not None and hasattr(section, "repeats"):
                    setattr(section, "repeats", repeats)
        ...
        if phase_a:
            logger.info(
                ...
                self.test_id, len(phase_a),
            )
            coros = [self._run_one(m) for m in phase_a]
            for spec, mod_results, err in await asyncio.gather(*coros):
                if err is not None:
                    logger.error(
                        "[%s] %s module failed: %s",
                        self.test_id, spec.name, err, exc_info=err,
                    )
                    ...
                    logger.info(
                        "[%s] %s: %d results",
                        self.test_id, spec.name, len(mod_results),
                    )
        ...
        for spec in phase_b:
            ...
            spec, mod_results, err = await self._run_one(spec)
            if err is not None:
                logger.error(
                    "[%s] %s module failed: %s",
                    self.test_id, spec.name, err, exc_info=err,
        ...
        if self.module_failures:
            logger.warning(
                ...
                self.test_id, len(self.module_failures),
                ", ".join(self.module_failures),
            )
        logger.info(
            "[%s] Probe complete. Total results: %d",
            self.test_id, len(results),
        )
        return results
    ...
    def save_report(
        ...
        report_data = {
            ...
            "summary": _summarize(
                results, module_failures=list(self.module_failures),
            ),
        }
